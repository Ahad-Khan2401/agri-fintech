...
serve(async (req) => {
  ...
  try {
    const { data: sale, error: saleError } = await supabase.from("sale_requests").select("*, livestock:livestock(*)").eq("id", saleId).single();
    ...
    if (profit <= 0) {
      ...
      const { data, error } = await supabase.rpc("distribute_profit", { p_sale_id: saleId, p_net_profit: netProfit, p_platform_fee: platformFee });
      if (error || !data?.success) throw error || new Error(data?.error || "Profit distribution failed");
    }

    const { error: saleUpdateError } = await supabase.from("sale_requests").update({ status: "approved", admin_verified_at: new Date().toISOString() }).eq("id", saleId);
    if (saleUpdateError) throw saleUpdateError;
    const { error: livestockUpdateError } = await supabase.from("livestock").update({ status: "sold" }).eq("id", sale.livestock.id);
